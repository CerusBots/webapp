...
const readStyleFile = (tplFilePath, href) => readFileSync(resolve(parse(tplFilePath).dir, href), 'utf8')
const defaultCompilerOptions = { client: true, strict: true }

const compilers = {
  css: readStyleFile,
  scss: (templateFile, href) => renderSync({
    data: readStyleFile(templateFile, href),
    importer: (url, prev) => ({ href: resolve(parse(prev === 'stdin' ? templateFile : prev).dir, url) })
  }).css.toString('utf8')
}
